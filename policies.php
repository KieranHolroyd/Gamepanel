...
<div class="modal" id="createGuide">
    ...
    <div class="content" style="max-width: 900px;padding:0;">
        <div class="field">
            ...
            <input id="Gtitle" type="text" class='fieldInput' placeholder="Policy Title">
        </div>
        <div class="field">
            <div class='fieldTitle'>Policy Body</div>
            <textarea id="guide_body"></textarea>
        </div>
...
<div class="modal" id="editGuide">
    ...
    <div class="content" style="max-width: 900px;padding:0;">
        <div class="field">
            <div class='fieldTitle'>Policy Title</div>
            <input id="Gtitle_edit" type="text" class='fieldInput' placeholder="Policy Title">
        </div>
        <div class="field">
            <div class='fieldTitle'>Policy Body</div>
            <textarea id="guide_body_edit"></textarea>
        </div>
...
<script>
    ...
    ClassicEditor.create(document.querySelector('#guide_body')).then(e => {
        editor = e;
    }).catch(error => {
        console.error(error);
    });
    ClassicEditor.create(document.querySelector('#guide_body_edit')).then(e => {
        edit_editor = e;
    }).catch(error => {
        console.error(error);
    });
    ...
    function getGuides() {
        ...
        $.get('api/v2/guide/get', function(data) {
            ...
            for (let guide of guides) {
                list += `<div class="selectionTab" onclick="getFullGuide(${guide.id})"><span style="float: right;font-size: 12px;">Effective: ${guide.effective}</span><span style="font-size: 25px;">${guide.title}<br></span></div>`;
            }
            $('#guides').html(list);
            checkIfSLT();
        });
