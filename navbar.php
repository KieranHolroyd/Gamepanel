...
<script>
    function getAllMildlyRelevantInformation() {
        ...
        $.get('https://arma3-servers.net/api/v1/?object=servers&element=detail&key=5p9s4QXs3QWE4MsL3h5WKBzTnYiohqlM9T', data => {
            data = JSON.parse(data);
            $('#server1players').text(`${data.players}/${data.maxplayers}`);
        });
        $.get('https://arma3-servers.net/api/v1/?object=servers&element=detail&key=rI7QeSl6jHuvwnvrQjV7KnFPLYNROcDpBj', data => {
            data = JSON.parse(data);
            $('#server2players').text(`${data.players}/${data.maxplayers}`);
        });
        $.get('https://teamspeak-servers.org/api/v1/?object=servers&element=detail&key=JjJjeLi6vAbRzMcbD02a0XMMGs4pZMRyK', data => {
            data = JSON.parse(data);
