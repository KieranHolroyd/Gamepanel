    ...
    if (data.code === 200) {
      ...
      if (!data.response.length)
        $("#notificationsDisplay").html(
          '<p class="overlayErrorMessage">No Notifications Found</p>'
        );
    } else {
      $("#notificationsDisplay").html(
        `<p class="overlayErrorMessage">${data.message}</p>`
      );
    }
  });
}
...
function parseNotification(n) {
  return `<div class="notification${
    n.viewed === "1" ? " viewed" : ""
  }" onclick="openCallback('${n.callback_url}')">
...
function openCallback(u) {
  if (
    u.charAt(1) === "/" &&
    confirm(`You're about to visit ${u} Are you sure?`)
  ) {
